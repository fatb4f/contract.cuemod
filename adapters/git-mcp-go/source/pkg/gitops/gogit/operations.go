...
// AmendCommit amends the most recent commit.
func (g *GoGitOperations) AmendCommit(repoPath string, message string, noEdit bool) (string, error) {
	...
	if noEdit {
		...
		return "", fmt.Errorf("message is required unless no_edit is true")
	}
...
// CherryPick applies the changes introduced by an existing commit.
func (g *GoGitOperations) CherryPick(repoPath string, revision string) (string, error) {
	output, err := gitops.RunGitCommand(repoPath, "cherry-pick", revision)
	if err != nil {
...
// RevertCommit creates a commit that reverts an existing commit.
func (g *GoGitOperations) RevertCommit(repoPath string, revision string, noCommit bool) (string, error) {
	args := []string{"revert"}
	if noCommit {
...
// ListWorktrees lists linked worktrees in porcelain format.
func (g *GoGitOperations) ListWorktrees(repoPath string) (string, error) {
	// go-git linked worktree support is not used in this slice because the repo
	// currently uses go-git v5. This keeps worktree support compatible with the
	// existing backend model.
	return gitops.RunGitCommand(repoPath, "worktree", "list", "--porcelain")
}

// AddWorktree creates a linked worktree.
func (g *GoGitOperations) AddWorktree(repoPath string, worktreePath string, branch string, commit string, detach bool) (string, error) {
	// go-git linked worktree support is not used in this slice because the repo
	// currently uses go-git v5. This keeps worktree support compatible with the
	// existing backend model.
...
func (g *GoGitOperations) RemoveWorktree(repoPath string, worktreePath string, force bool) (string, error) {
	// go-git linked worktree support is not used in this slice because the repo
	// currently uses go-git v5. This keeps worktree support compatible with the
	// existing backend model.
...
func (g *GoGitOperations) PruneWorktrees(repoPath string) (string, error) {
	// go-git linked worktree support is not used in this slice because the repo
	// currently uses go-git v5. This keeps worktree support compatible with the
	// existing backend model.
	output, err := gitops.RunGitCommand(repoPath, "worktree", "prune")
	if err != nil {
