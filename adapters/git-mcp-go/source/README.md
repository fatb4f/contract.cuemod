...
- **git_worktree_list**: Lists linked worktrees in porcelain format
- **git_worktree_add**: Adds a linked worktree
- **git_worktree_remove**: Removes a linked worktree
- **git_worktree_prune**: Prunes stale worktree administrative files
...
The `--write-access` flag enables operations that modify remote state (currently only the push operation). Local worktree operations do not require this flag.
...
- **allow-read-only**: Auto-approve all read-only tools (git_status, git_diff_unstaged, git_diff_staged, git_log, git_show, git_diff, git_worktree_list)
- **allow-local-only**: Auto-approve all local-only tools (incl. git_commit, git_add, git_reset, git_worktree_add, git_worktree_remove, git_worktree_prune, but not git_push)
...
The `git_worktree_list`, `git_worktree_add`, `git_worktree_remove`, and `git_worktree_prune` tools use Git CLI-compatible behavior:

- `git_worktree_list`: `git worktree list --porcelain`
- `git_worktree_add`: `git worktree add [--detach] [-b branch] <path> [commit]`
- `git_worktree_remove`: `git worktree remove [--force] <path>`
- `git_worktree_prune`: `git worktree prune`

In `go-git` mode, worktree operations currently fall back to the Git CLI.
